...
if (!CLIENT_SECRET) {
  console.error("HR_CLIENT_SECRET missing in .env");
  process.exit(1);
}
...
(async () => {
  ...
  console.log(`[hr] ready: http://localhost:${PORT}`);
})();
...
// Active la protection globale HR
app.use(requireHrAccess);
...
app.get("/callback", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("[hr] Event collector error:", e);
    }
...
app.listen(PORT, () => console.log(`HR running on http://localhost:${PORT}`));
